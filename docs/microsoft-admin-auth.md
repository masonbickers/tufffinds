...
authorisation. Microsoft sign-in does not grant admin access by itself. Every
administrator must also have an active `admin_users/{uid}` document whose `role`
is `admin`.
...
## 4. Authorize the website domains
...
## 5. Approve each administrator by Firebase UID
...
- An authenticated but unapproved Microsoft account is denied.
- An approved `admin_users/{uid}` account can open `/admin` and
  `/admin/email-signups`.
- No signup data is requested before the admin record is approved.
...
accounts. Disable it only after confirming no active administrator relies on a
Google-linked Firebase identity.
